...
#define Si5351_CAL 125000 // Calibrate for your Si5351 module.
#define FREQ_BFO 452500   // in Hz: Calibrate for your IF Filter.
...
#define FREQ_0 14074000 // RF freqency in Hz
#define FREQ_1 14074000 // in Hz
...
#define pin_SW 4     //pin for freq change switch (D10,input)
...
#define VOX_LEVEL 15000 // min input (from PC) audio level to trigger transmit
...
MbedI2C Wire1(6,7); // for XIOA RP2040
#define WIRE Wire1

#include <si5351.h>  //Si5351 Library for Arduino, https://github.com/etherkit/Si5351Arduino
Si5351 si5351(SI5351_BUS_BASE_ADDR, WIRE); // note added parameter for which i2c bus
...
void setup() {
  ...
  delay(200);
  // loop until clock responds 
  while (1) {
  bool res = si5351.init(SI5351_CRYSTAL_LOAD_10PF, XTAL_FREQ, 0); 
  if (res) {
    break; 
  } 
  else {
    digitalWrite(pin_RED, LOW); // flash red
    delay(500);
...
void receiving() {
 ...
 //   if (monomax > 22938)
    if (monomax > VOX_LEVEL)
    {                       // VOX if signal exceeds 70% of 2^15
      Tx_Start = 1;
      not_TX_first = 0;
         // MPM - go blue for transmit
...
int16_t adc() {
  ...
  return int(adc/24); // avoid overflow and the average the data
}
